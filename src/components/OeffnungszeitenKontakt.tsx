        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-20 items-start">
          ...
          <Reveal>
            <p className="sec-label mb-4">
              <span style={{ color: 'rgba(61,158,191,0.4)', marginRight: '0.5rem', fontFamily: 'var(--font-playfair)' }}>05</span>
              Wo wir sind
            </p>
            <h2 className="font-display font-bold text-[clamp(2rem,4vw,3rem)]
              leading-[1.1] mb-3" style={{ color: '#EEF2F7' }}>
              Besuche <span className="steel-text">uns</span>
            </h2>
              ...
              {[
                {
                  ...
                  label: 'Adresse',
                  content: <a href={salonConfig.adresse.googleMapsUrl} target="_blank" rel="noopener noreferrer" className="hover:text-[#3D9EBF] transition-colors" style={{ color: '#7A8BA0' }}>{salonConfig.adresse.strasse}<br/>{salonConfig.adresse.plz} {salonConfig.adresse.stadt}</a>,
                },
                {
                  ...
                  label: 'Telefon',
                  content: <a href={salonConfig.kontakt.telefonHref} className="hover:text-[#3D9EBF] transition-colors" style={{ color: '#7A8BA0' }}>{salonConfig.kontakt.telefon}</a>,
                },
                {
                  ...
                  label: 'Instagram',
                  content: <a href={salonConfig.kontakt.instagramUrl} target="_blank" rel="noopener noreferrer" className="hover:text-[#3D9EBF] transition-colors" style={{ color: '#7A8BA0' }}>{salonConfig.kontakt.instagram}</a>,
                },
              ].map(({ icon, label, content }) => (
                <div key={label} className="flex items-start gap-3.5">
                  <div className="w-[42px] h-[42px] rounded-[12px] flex items-center justify-center flex-shrink-0"
                    style={{ background: 'rgba(61,158,191,0.08)', border: '1px solid rgba(61,158,191,0.15)', color: '#3D9EBF' }}>
                    {icon}
                  </div>
                  <div>
                    <div className="text-[0.85rem] font-medium mb-0.5" style={{ color: '#EEF2F7' }}>{label}</div>
          ...
          {/* Öffnungszeiten */}
          <Reveal delay={150}>
            <h3 className="font-display font-bold text-[1.35rem] mb-6" style={{ color: '#EEF2F7' }}>
              Öffnungszeiten
            </h3>
              ...
              {salonConfig.oeffnungszeiten.map((row) => {
                ...
                return (
                  <li key={row.tag}
                    ...
                    style={{
                      background: isToday ? 'rgba(61,158,191,0.08)'  : 'transparent',
                      border:     isToday ? '1px solid rgba(61,158,191,0.2)' : '1px solid transparent',
                    }}
                  >
                    <span className="text-[0.85rem] flex items-center gap-2.5"
                      style={{ color: isToday ? '#3D9EBF' : '#7A8BA0' }}>
                      {row.tag}
                      {isToday && (
                        <span className="text-[0.58rem] tracking-[0.08em] uppercase px-2 py-[0.18rem] rounded-full"
                          style={{ background: 'rgba(61,158,191,0.18)', color: '#3D9EBF' }}>
                          Heute
                        </span>
                      )}
                    </span>
                    <span className="text-[0.85rem] font-medium"
                      style={{ color: isToday ? '#3D9EBF' : row.geschlossen ? 'rgba(238,242,247,0.2)' : '#EEF2F7' }}>
                      {row.zeit}
                    </span>
                  </li>
                )
