      ...
      <div className="max-w-salon mx-auto px-[clamp(1.5rem,5vw,4rem)]">
        ...
        <Reveal className="mb-10">
          <p className="sec-label mb-4">
            <span style={{ color: 'rgba(184,115,51,0.4)', marginRight: '0.5rem', fontFamily: 'var(--font-playfair)' }}>01</span>
            Preisliste
          </p>
          ...
          <p className="text-[0.93rem] leading-[1.85] max-w-[52ch]" style={{ color: '#8A7E72' }}>
        ...
        <Reveal delay={100} className="mb-6">
          <div className="flex border-b" style={{ borderColor: 'rgba(237,232,225,0.07)' }} role="tablist">
            {salonConfig.leistungen.map((l) => (
              <button key={l.id} role="tab"
                ...
                style={{ color: activeId === l.id ? '#EDE8E1' : '#8A7E72' }}
              >
                {l.kategorie}
                {activeId === l.id && (
          ...
          <div id={`panel-${active.id}`} role="tabpanel">
            {active.items.map((item, i) => (
              <div key={i}
                ...
                style={{ borderBottom: '1px solid rgba(237,232,225,0.05)' }}
              >
                <div>
                  <div className="text-[0.93rem]" style={{ color: '#EDE8E1' }}>{item.name}</div>
                  {item.tag && (
                    <div className="text-[0.75rem] mt-0.5" style={{ color: '#8A7E72' }}>{item.tag}</div>
                  )}
                </div>
